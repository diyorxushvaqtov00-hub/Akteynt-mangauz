...
export default function TranslatePage() {
  ...
  async function submit() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || "Faylni tekshirishda xatolik yuz berdi.")
      setMessage(`${result.fileName} qabul qilindi. ${result.sizeMB} MB, ${result.pages ?? "?"} sahifa. Keyingi bosqich: sahifalarni ajratish va OCR.`)
    } catch (e) {
    ...
    <main className="min-h-screen bg-[#0b0a0f] px-4 py-12 text-white">
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mt-8 rounded-3xl border border-white/10 bg-white/[0.03] p-6 shadow-2xl sm:p-10">
          ...
          <p className="mt-3 leading-7 text-white/65">Manga yoki manhwa bobingizni PDF shaklida yuklang. Hozirgi bosqich PDF faylni tekshiradi; keyin sahifalarni ajratish, OCR va o‘zbekchaga tarjima qilishni ulaymiz.</p>
          ...
          <button type="button" disabled={!file || busy} onClick={submit} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-purple-600 px-5 py-3.5 font-semibold transition hover:bg-purple-500 disabled:cursor-not-allowed disabled:opacity-40">
            {busy && <LoaderCircle size={18} className="animate-spin" />}{busy ? "Tekshirilmoqda..." : "PDF faylni yuklash"}
          </button>
          <p className="mt-4 text-xs leading-5 text-white/40">Hozircha fayl turi va hajmi tekshiriladi. AI tarjimasi hamda doimiy saqlash hali ulanmagan.</p>
        </div>
      </div>
    </main>
  )
