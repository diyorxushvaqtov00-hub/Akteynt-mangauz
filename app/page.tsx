...
import { trending as demoTrending, ranking as demoRanking } from "@/lib/manga-data"
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[200px_minmax(0,1fr)_280px]">
          <div className="order-2 lg:order-1">
            <div className="lg:sticky lg:top-20"><GenreSidebar /></div>
          </div>
          <div className="order-1 flex flex-col gap-10 lg:order-2">
            <HeroSlider items={trending.slice(0, 3)} />
            <MangaGrid id="trending" title="Trending Now" icon={<Flame className="size-5 text-primary" />} items={trending} />
            <MangaGrid id="new" title="New Releases" icon={<Sparkles className="size-5 text-primary" />} items={newReleases} />
          </div>
          <div id="rankings" className="order-3 scroll-mt-20">
            <div className="lg:sticky lg:top-20"><RankingList items={ranking.length ? ranking : demoRanking} /></div>
          </div>
        </div>
      </main>
      <SiteFooter />
